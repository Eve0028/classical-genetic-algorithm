...
def objective(trial: optuna.trial.Trial, function: callable, num_variables: int, start_interval: float,
    ...
    num_generations = trial.suggest_int('num_generations', 500, 2000)
    ...
    tournament_size = trial.suggest_int('tournament_size', 2,
                                        3) if selection_strategy_type == 'TOURNAMENT' else None
    ...
    elite_size = trial.suggest_int('elite_size', population_size // 100, population_size // 20) if elite_strategy else 0

    # crossover_strategy_type = trial.suggest_categorical('crossover_strategy', ['POINT', 'DISCRETE', 'UNIFORM'])
    crossover_strategy_type = trial.suggest_categorical('crossover_strategy', ['POINT', 'UNIFORM'])
    crossover_probability = trial.suggest_float('crossover_probability', 0.8, 0.9)
    ...
    mutation_strategy_type = trial.suggest_categorical('mutation_strategy', ['ONE_POINT', 'TWO_POINT'])
    mutation_probability = trial.suggest_float('mutation_probability', 0.01, 0.1)
    ...
    if is_inversion:
        inversion_probability = trial.suggest_float('inversion_probability', 0.05, 0.3)
...
def run_study(database_url: str, function: callable, function_name: str, num_variable: int, precision: int,
    ...
    study = optuna.create_study(
        study_name=f'{function_name}__var_{num_variable}__prec_{precision}__specific_args',
        direction='minimize',
    ...
    study.optimize(partial_objective, n_trials=15)
